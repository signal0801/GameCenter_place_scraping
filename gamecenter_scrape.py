...
def drivePage(pref,page):
    ...
    driver.get(base_url + "/game/facility/e-AMUSEMENT/p/result.html?finder=area&gkey=JUBEATPROP&area=-1&pref="+str(pref)+"&page="+str(page))
...
for pref in range(1, 48):
    ...
    for shopdic in pref_shopdic_list:
      ...
      conn.cursor().execute("insert into shop(id,name,pref,address,x,y,paseli) VALUES(NULL,?,NULL,?,NULL,NULL,?)",(shopdic["Name"], shopdic["Address"], shopdic["PASELI"],))
      conn.commit()
